...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (label[0] == '@') {
        sLabel = sLabel.substr(1, sLabel.length());
        fullbutton = true;
    }
    ...
    ImVec2 label_size = CalcTextSize(sLabel.c_str(), NULL, true) / 2;
   ...
   //     flags |= ImGuiButtonFlags_Repeat;
    ...
    // Render
    const ImU32 col = GetColorU32((held && hovered) ? ColorConvertFloat4ToU32(ImVec4(0.15f, 0.15f, 0.15f, 1.f)) : hovered ? ColorConvertFloat4ToU32(ImVec4(0.2f, 0.2f, 0.2f, 1.f)) : ColorConvertFloat4ToU32(ImVec4(0.2f, 0.2f, 0.2f, 1.f)));
    ...
    bb.Min += ImVec2(0, 2);
    bb.Max += ImVec2(0, 6);
    const ImColor textcolor = (held && hovered) ? ImColor(20, 20, 20,255) : hovered ? ImColor(30, 30, 30,255) : ImColor(37, 37, 37, 255);
    auto draw = GetCurrentWindow()->DrawList;

    if (fullbutton) {
        draw->AddRectFilled(bb.Min, bb.Max, textcolor, true, 3.f);
        draw->AddRect(ImVec2(bb.Min.x - 1, bb.Min.y + 1), ImVec2(bb.Max.x + 1, bb.Max.y - 1), ImColor(60, 60, 60, 255), true, 3.f);
    }
    ...
    float addy = CalcTextSize(sLabel.c_str()).y / 4;
    RenderTextClipped(bb.Min  + ImVec2(0, addy), bb.Max - style.FramePadding, sLabel.c_str(), NULL, &label_size, style.ButtonTextAlign, &bb);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawFlags flags)
{
    ...
    ImRect bb = bb_frame;
    bb.Expand(ImVec2(-ImClamp(IM_FLOOR((bb_frame_width - 2.0f) * 0.5f), 0.0f, 3.0f), -ImClamp(IM_FLOOR((bb_frame_height - 2.0f - 35) * 0.5f), 0.0f, 3.0f)));
    ...
    const float win_size_v = ImMax(ImMax(size_contents_v, size_avail_v), 1.1f);
    ...
    float scroll_max = ImMax(1.1f, size_contents_v - size_avail_v);
    ...
    // Render
    const ImU32 bg_col = ImColor(20, 20, 20, 200);  //GetColorU32(ImGuiCol_ScrollbarBg);
    const ImU32 grab_col = ImColor(70, 70, 70, 255);//GetColorU32(held ? ImGuiCol_ScrollbarGrabActive : hovered ? ImGuiCol_ScrollbarGrabHovered : ImGuiCol_ScrollbarGrab, alpha);

    ImRect frame = bb_frame;
    int moveLeft = 4;
    frame.Max.y += 8;
    frame.Min.x += 2 - moveLeft;
    frame.Max.x -= 2 + moveLeft;

    
    window->DrawList->AddRectFilled(frame.Min, frame.Max, bg_col, 2.f);
    window->DrawList->AddRect(frame.Min, frame.Max, ImColor(100, 100, 100,50), 2.f);
    ...
    window->DrawList->AddRectFilled(ImVec2(grab_rect.Min.x + 1 - moveLeft, grab_rect.Min.y), ImVec2(grab_rect.Max.x - 1 - moveLeft, grab_rect.Max.y + 8), grab_col, 2.f);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect check_bb(pos + ImVec2(2, 2), pos + ImVec2(13, 13));

    const ImRect total_bb(check_bb.Min - ImVec2(3, 3), check_bb.Max + ImVec2(label_size.x / 2 + 10, 3));
    ...
    //window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ((held && hovered) ? ImColor(80, 80, 80, 255) : hovered ? ImColor(70, 70, 70, 255) : ImColor(60, 60, 60, 255)), true, 2.f);
    //window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ((held && hovered) ? ImColor(80,80,80, 255) : hovered ? ImColor(70, 70, 70, 255) : ImColor(60, 60, 60, 255)), true, 2.f);
    RenderFrame(check_bb.Min, check_bb.Max, ((held && hovered) ? ImColor(80, 80, 80, 255) : hovered ? ImColor(70, 70, 70, 255) : ImColor(60, 60, 60, 255)), true, 1.f);
    ...
    {
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(1,1), check_bb.Max - ImVec2(1, 1), utils::FloatToImColor(settings::misc::menu::color), true, 2.f);
        //window->DrawList->AddRect(check_bb.Min - ImVec2(1, 1), check_bb.Max + ImVec2(1, 1),ImColor(100, 100, 100, 100), true, 2.f);
        //const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
        //RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), check_col, square_sz - pad * 2.0f);
    }

    ImVec2 label_pos = ImVec2(check_bb.Max.x + 12, check_bb.Min.y - 2);
    if (g.LogEnabled)
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const float size = GetWindowWidth() - 43;

    const ImRect rect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(size - ImGui::GetCursorPos().x, 55));
    const ImRect clickable(window->DC.CursorPos + ImVec2(0, 23), window->DC.CursorPos + ImVec2(size - ImGui::GetCursorPos().x, 53));

    const ImVec2 arrow_size = gui::combo_arrow->CalcTextSizeA(9.0f, FLT_MAX, 0.0f, "A");
    ...
    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, ImColor(40, 40, 40), 3.0f);
    window->DrawList->AddRect(clickable.Min, clickable.Max, ImColor(1.0f, 1.0f, 1.0f, 0.1f), 3.0f);
    char weapIcon[2] = { label[1] };
    std::string sLabel(label);
    
    window->DrawList->AddText(gui::menufont, gui::menufontsz,rect.Min, GetColorU32(ImGuiCol_Text), sLabel.c_str());
   

    //std::string pv(preview_value);
    //pv = "?" + pv;
    RenderTextClipped(clickable.Min + ImVec2(14, 7), clickable.Max - ImVec2(24, -47), preview_value, NULL, NULL, ImVec2(0.0f, 0.0f));

    PushFont(gui::combo_arrow); {
        window->DrawList->AddText(ImVec2(clickable.Max.x - 20, (clickable.Min.y + clickable.Max.y) / 2 - arrow_size.y / 2), ImColor(1.0f, 1.0f, 1.0f, 1.f), "A");
    } PopFont();
    ...
    PushStyleColor(ImGuiCol_Border, ImVec4(1.0f, 1.0f, 1.0f, 0.1f));
    PushStyleColor(ImGuiCol_PopupBg, ImVec4(ImColor(30, 30, 30)));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const float w = GetWindowWidth() - 70;

    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y / 2 + 10));
    const ImRect frame_bb(total_bb.Min + ImVec2(0, label_size.y / 2 + 5), total_bb.Max);
    ...
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(frame_bb, id, data_type, p_data, p_max, p_min, format, flags, &grab_bb);
    if (value_changed)
        MarkItemEdited(id);
    ...
    window->DrawList->AddRect(frame_bb.Min - ImVec2(1,1), frame_bb.Max + ImVec2(1, 1) - ImVec2(12, 0), ImColor(20, 20, 20), 1.0f);
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max - ImVec2(12,0), ImColor(62, 62, 62), 1.0f);
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(grab_bb.Min.x, frame_bb.Max.y), utils::FloatToImColor(settings::misc::menu::color), 1.0f);
    //window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + 5, grab_bb.Min.y), 6.0f, ImColor(1.0f, 1.0f, 1.0f), 30);

    RenderText(total_bb.Min, label);


    PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 1.0f, 1.0f, 1.f)); {

        if (data_type == ImGuiDataType_S32) {
            int* value = (int*)(p_data);
            float percent = float(*value * 100 / 500);
            
            std::stringstream b;
            b << value;
            gui::DrawOutlinedText(gui::bold, 15.f, ImVec2(grab_bb.Min.x - CalcTextSize(b.str().c_str()).x /4 + 20 , frame_bb.Max.y- 6), ImColor(225, 255, 255), value_buf);
            std::stringstream a;
            

           
            //RenderText(, value_buf);
        }
        if (data_type == ImGuiDataType_Float) {
            float* value = (float*)(p_data);
            float percent = float(*value * 100 / 500);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (!is_multiline)
    {
        ...
        window->DrawList->AddRectFilled(frame_bb.Min , frame_bb.Max + ImVec2(2,0), ImColor(30, 30, 30), true, style.FrameRounding);
        window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(3, 3), frame_bb.Max - ImVec2(3, 3), ImColor(40, 40, 40), true, style.FrameRounding);
        
        window->DrawList->AddRect(frame_bb.Min - ImVec2(1,1), frame_bb.Max + ImVec2(1, 1), ImColor(70, 70, 70), true, style.FrameRounding);
    }
...
static void RenderArrowsForHorizontalBar(ImDrawList* draw_list, ImVec2 pos, ImVec2 half_sz, float bar_w, float alpha)
{
    ImU32 alpha8 = IM_F32_TO_INT8_SAT(alpha);
    ImGui::RenderArrowPointingAt(draw_list, ImVec2(pos.x , pos.y + half_sz.y + 1), ImVec2(half_sz.x + 2, half_sz.y + 1), ImGuiDir_Up, IM_COL32(255, 255, 255, alpha8));
    ImGui::RenderArrowPointingAt(draw_list, ImVec2(pos.x , pos.y + +half_sz.y), half_sz, ImGuiDir_Up, IM_COL32(255, 255, 255, alpha8));
    ImGui::RenderArrowPointingAt(draw_list, ImVec2(pos.x , pos.y + bar_w - half_sz.x - 1), ImVec2(half_sz.x + 2, half_sz.y + 1), ImGuiDir_Down, IM_COL32(0, 0, 0, alpha8));
    ImGui::RenderArrowPointingAt(draw_list, ImVec2(pos.x , pos.y + bar_w - half_sz.x), half_sz, ImGuiDir_Down, IM_COL32(0, 0, 0, alpha8));
}
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    float sv_picker_size = ImMax(bars_width * 1, width - (false ? 2 : 1) * (bars_width + style.ItemInnerSpacing.x)) / 3; // Saturation/Value picking box
    ...
    if (alpha_bar)
    {
        ...
        ImVec2 alphabarStart = ImVec2(0, sv_picker_size + style.ItemInnerSpacing.y) + picker_pos;
        ImVec2 alphabarEnd = ImVec2(sv_picker_size, sv_picker_size + style.ItemInnerSpacing.y + bars_width) + picker_pos;

        SetCursorScreenPos(alphabarStart);
        ...
        //float bar0_pos_y = picker_pos.y + style.ItemInnerSpacing.y;
        //
        //
        //SetCursorScreenPos(ImVec2(picker_pos.x, bar0_pos_y));
        //InvisibleButton("alpha", ImVec2(sv_picker_size, bars_width));
        //if (IsItemActive())
        //{
    ...
    //    draw_list->AddCircleFilled(hue_cursor_pos, hue_cursor_rad / 2, hue_color32, hue_cursor_segments);
    //    draw_list->AddCircle(hue_cursor_pos, hue_cursor_rad / 2 + 1, col_midgrey, hue_cursor_segments);
    //    draw_list->AddCircle(hue_cursor_pos, hue_cursor_rad / 2, col_white, hue_cursor_segments);
    //
    ...
    if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        ImVec2 alphabarStart = ImVec2(picker_pos.x + sv_picker_size + style.ItemInnerSpacing.x, bar0_line_y);
        ImVec2 alphabarEnd = ImVec2(picker_pos.x + style.ItemInnerSpacing.x + sv_picker_size + bars_width, bar0_line_y + 2);

        draw->AddRect(alphabarStart - ImVec2(1,1), alphabarEnd + ImVec2(1, 1), ImColor(0, 0, 0, 255));

        draw->AddRectFilled(alphabarStart, alphabarEnd, ImColor(255, 255, 255, 255));
    ...
    draw_list->AddCircleFilled(sv_cursor_pos, sv_cursor_rad / 1.5, user_col32_striped_of_alpha, 12);
    draw_list->AddCircle(sv_cursor_pos, sv_cursor_rad / 1.5 - 1, ImColor(0,0,0,255), 12);
    draw_list->AddCircle(sv_cursor_pos, sv_cursor_rad / 1.5, col_white, 12);

    // Render alpha bar
    if (alpha_bar)
    {

        //         draw->AddRectFilled(picker_pos + ImVec2(0,  +  sv_picker_size + style.ItemInnerSpacing.y), picker_pos +  ImVec2(sv_picker_size ,  + style.ItemInnerSpacing.y + sv_picker_size + bars_width ), ImColor(255, 255, 255, 255));
        ...
        draw_list->AddRectFilled(alphabarStart - ImVec2(1,1), alphabarEnd + ImVec2(1, 1), ImColor(0, 0, 0, 255));

        draw_list->AddRectFilledMultiColor(alphabarStart, alphabarEnd, user_col32_striped_of_alpha & ~IM_COL32_A_MASK, user_col32_striped_of_alpha, user_col32_striped_of_alpha, user_col32_striped_of_alpha & ~IM_COL32_A_MASK);

        draw->AddRect(ImVec2(bar1_line_x, alphabarStart.y) - ImVec2(1, 1), ImVec2(bar1_line_x + 2, alphabarEnd.y) + ImVec2(1, 1), ImColor(0, 0, 0, 255));


        draw->AddRectFilled(ImVec2(bar1_line_x, alphabarStart.y), ImVec2(bar1_line_x + 2, alphabarEnd.y), ImColor(255, 255, 255, 255));
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    float default_size = GetFrameHeight() / 2;
    if (size.x == 0.0f)
        size.x = default_size;
    if (size.y == 0.0f)
        size.y = default_size;
    ...
    const ImRect bb(window->DC.CursorPos , window->DC.CursorPos + ImVec2(24, 12));
    ...
    float rounding = 1; //ImMin(g.Style.FrameRounding, grid_step * 0.5f);
    ImRect bb_inner = bb;
    ...
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
        if (g.Style.FrameBorderSize > 0.0f)
            ...
            window->DrawList->AddRect(bb.Min, bb.Max, ImColor(0,0,0,255), rounding); // Color button are often in need of some sort of border
        else
            window->DrawList->AddRect(bb.Min, bb.Max, ImColor(0, 0, 0, 255), rounding); // Color button are often in need of some sort of border
    }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    ImGuiID id = window->GetID(label);
    ImVec2 label_size = CalcTextSize(label, NULL, true) / 2;
    ...
    if (hovered || selected)
    {
        const ImColor col = ((held && hovered) ? ImColor(60, 60, 60) : hovered ? ImColor(70, 70, 70) : ImColor(50, 50, 50));
    ...
    RenderTextClipped(text_min, text_max, txt.c_str(), NULL, &label_size, style.SelectableTextAlign, &bb);
